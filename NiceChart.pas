...
type
  ...
  private
    ...
    FMonochrome: Boolean;
    FSoftColors: Boolean;
    FFormatterXAxis: string;
    FFormatterYAxis: string;
    procedure InternalClear;
    ...
    function GetLabel(Value: Double; Formatter: string): string;
  ...
  published
    ...
    property FormatterXAxis: string read FFormatterXAxis write FFormatterXAxis;
    property FormatterYAxis: string read FFormatterYAxis write FFormatterYAxis;
...
function TNiceSeries.GetCount: Integer;
begin
  Result := Values.Count;
end;
...
begin
  ...
  YAxis               := TList.Create;
  FUpdating           := False;
  FFormatterXAxis     := Formatter;
  FFormatterYAxis     := Formatter;
end;
...
begin
  ...
  if (fsBold in ACanvas.Font.Style) then
    ...
    FBold := FW_NORMAL;
  RotFont := CreateFont(ACanvas.Font.Height, 0, Angle * 10, 0, FBold, FItalic, FUnderline, FStrikeOut, 1, 4, $10,
...
begin
  ...
  begin
    ...
    DrawText(Canvas.Handle, PChar(FTitle), Length(FTitle), RcTitle, DT_CENTER or DT_VCENTER or DT_WORDBREAK or DT_CALCRECT);
...
function TNiceChart.GetLabel(Value: Double; Formatter: string): string;
begin
...
begin
  ...
  for x    := 0 to List.Count - 1 do
  begin
    if (TNiceSeries(List[x]).FKind = skBar) then
      Inc(BarCount);
  end;
  if (BarCount > 0) then
  ...
  begin
    ...
    begin
      ...
      P^.Caption := GetLabel(l / FAxisXScale, FFormatterXAxis);
      XAxis.Add(P);
      Last := l;
      Inc(i);
    end;
  ...
  begin
    ...
    begin
      ...
      P^.Caption := GetLabel((Lowest + l) / FAxisXScale, FFormatterXAxis);
      XAxis.Add(P);
    end;
...
begin
  ...
  begin
    ...
    P^.Value   := Lowest + t;
    P^.Py      := Round(t * Scale);
    P^.Caption := GetLabel((Lowest + t) / FAxisYScale, FFormatterYAxis);
    YAxis.Add(P);
  end;
...
const
  Colors1: array [0 .. 13] of TColor = (clRed, clBlue, clGreen, clFuchsia, clNavy, clMaroon, clBlack, clOlive, clPurple, clTeal,
    clGray, clLime, clYellow, clAqua);
  Colors2: array [0 .. 13] of TColor = ($0066C2FF, $005AFADA, $00F4C84D, $00B54DF4, $00669FFF, $00F44D5A, $0066E0FF, $0066FFFF,
    $00F44DAE, $006863FE, $004DF474, $00F4934D, clSilver, clGray);

  Markers: array [0 .. 4] of TMarkerProc = (MarkerRectangle, MarkerCircle, MarkerTriangle1, MarkerTriangle2, MarkerDiamond);
...
begin
  ...
  if ((RcChart.Right - RcChart.Left) < (RcChart.Bottom - RcChart.Top)) then
    AWidth := AWidth + ((RcChart.Bottom - RcChart.Top) - (RCChart.Right - RcChart.Left));
